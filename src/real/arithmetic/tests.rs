...
mod tests {
    ...
    #[test]
    fn exact_rational_linear_form4_reuses_common_numerator_content() {
        ...
        let coefficients = [
            &Rational::new(6) * &coefficient_scale,
            &Rational::new(-10) * &coefficient_scale,
            Rational::zero(),
            &Rational::new(14) * &coefficient_scale,
        ];
        let exact = ExactRationalLinearForm4::from_rationals(coefficients.each_ref())
            .expect("the coefficients share nontrivial numerator content");
        let wide_point = [
            scale.clone(),
            scale.clone(),
            Rational::zero(),
        ...
        assert!(
            ExactRationalLinearForm4::should_retain(
        ...
        for point in [
            ...
            let refs = point.each_ref();
            let expected = match Rational::signed_product_sum_ordering(
                [true; 4],
                [
                    [&coefficients[0], refs[0]],
                    [&coefficients[1], refs[1]],
                    [&coefficients[2], refs[2]],
                    [&coefficients[3], refs[3]],
                ],
            ) {
            ...
            assert_eq!(exact.sign_rationals(refs), Some(expected));
        }

        let non_dyadic_point = [
            Rational::new(1) / Rational::new(3),
            Rational::one(),
            Rational::zero(),
            Rational::one(),
        ];
        assert_eq!(exact.sign_rationals(non_dyadic_point.each_ref()), None);
        assert!(
            !ExactRationalLinearForm4::should_retain(
                coefficients.each_ref(),
                non_dyadic_point.each_ref(),
            )
        );

        let non_dyadic_coefficients = [
            &coefficients[0] / Rational::new(5),
            coefficients[1].clone(),
            coefficients[2].clone(),
            coefficients[3].clone(),
        ];
        assert!(
            ExactRationalLinearForm4::from_rationals(non_dyadic_coefficients.each_ref()).is_none()
        );
        ...
        assert!(ExactRationalLinearForm4::from_rationals(coprime.each_ref()).is_none());
    }
